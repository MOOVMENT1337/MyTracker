...
// Shared activity and query helpers
export async function logActivity(db: Queryable, actorId: string, msg: string) {
  await db.query("INSERT INTO activity_log (actor_id,msg) VALUES ($1,$2)", [
    actorId,
    msg,
  ]);
}
...
export async function getIssue(db: Queryable, id: string) {
  const issue = required(
    (await db.query(`SELECT ${issueColumns} FROM issues i WHERE i.id=$1`, [id]))
      .rows[0],
    "Issue",
  );
  issue.comments = (
    await db.query(
      `SELECT ${commentColumns} FROM comments c JOIN users u ON u.id=c.author_id WHERE c.issue_id=$1 ORDER BY c.created_at,c.id`,
      [id],
    )
  ).rows;
  return issue;
...
export function buildWhere(filters: Filters, userId: string) {
  ...
  if (filters.search) {
    ...
    clauses.push(
      `(i.summary ILIKE $${values.length} OR i.description ILIKE $${values.length} OR i.key ILIKE $${values.length})`,
    );
  }
  return {
...
export async function listIssues(
  ...
  return transaction(pool, async (db) => {
    ...
    const total = Number(
      (await db.query(`SELECT count(*) FROM issues i ${sql}`, values)).rows[0]
        .count,
    );
    const issues = (
      await db.query(
        `SELECT ${issueColumns} FROM issues i ${sql}
      ORDER BY i.updated_at DESC,i.id LIMIT $${values.length + 1} OFFSET $${values.length + 2}`,
        [...values, filters.limit, filters.offset],
      )
    ).rows;
    const comments = (
      await db.query(
        `SELECT ${commentColumns} FROM comments c JOIN users u ON u.id=c.author_id
      WHERE c.issue_id=ANY($1::text[]) ORDER BY c.created_at,c.id`,
        [issues.map((i) => i.id)],
      )
    ).rows;
    const grouped = new Map<string, Row[]>();
    for (const comment of comments)
      grouped.set(comment.issueId, [
        ...(grouped.get(comment.issueId) || []),
        comment,
      ]);
...
export async function stats(pool: Database, filters: Filters, actorId: string) {
  ...
  const rows = (
    await pool.query(
      `SELECT status,count(*)::int AS count FROM issues i ${sql} GROUP BY status`,
      values,
    )
  ).rows;
...
export async function createQueue(
  ...
  return transaction(pool, async (db) => {
    const queue = (
      await db.query(
        "INSERT INTO queues (id,name,key,color) VALUES ($1,$2,$3,$4) RETURNING *",
        [randomUUID(), input.name, input.key, input.color],
      )
    ).rows[0];
    await logActivity(
      db,
      actorId,
      `Queue "${queue.name}" (${queue.key}) created`,
    );
    return queue;
  });
}
...
export async function deleteQueue(
  ...
  return transaction(pool, async (db) => {
    ...
    const queue = required(
      (await db.query("SELECT * FROM queues WHERE id=$1 FOR UPDATE", [id]))
        .rows[0],
      "Queue",
    );
    const issueCount = Number(
      (await db.query("SELECT count(*) FROM issues WHERE queue_id=$1", [id]))
        .rows[0].count,
    );
    ...
    await logActivity(
      db,
      actor.id,
      `Queue "${queue.name}" (${queue.key}) deleted with ${issueCount} issue(s)`,
    );
...
export async function createIssue(
  ...
  return transaction(pool, async (db) => {
    const queue = required(
      (
        await db.query(
          "UPDATE queues SET counter=counter+1 WHERE id=$1 RETURNING key,counter",
          [input.queueId],
        )
      ).rows[0],
      "Queue",
    ...
    await db.query(
      `INSERT INTO issues (id,key,queue_id,summary,description,status,priority,type,assignee_id,reporter_id)
...
export async function updateIssue(
  ...
  return transaction(pool, async (db) => {
    ...
    if (changes.version !== undefined && old.version !== changes.version)
      throw new ApiError(
        409,
        "VERSION_CONFLICT",
        "Issue changed; reload before saving",
      );
    const statusChanged =
    ...
    await db.query(
      `UPDATE issues SET ${assignments.join(",")} WHERE id=$1`,
      values,
    );
    if (statusChanged)
      await logActivity(
        db,
        actorId,
        `"${old.key}" status changed: ${old.status} → ${changes.status}`,
      );
    return getIssue(db, id);
  });
}
...
export async function deleteIssue(pool: Database, actorId: string, id: string) {
  return transaction(pool, async (db) => {
    const issue = required(
      (await db.query("DELETE FROM issues WHERE id=$1 RETURNING key", [id]))
        .rows[0],
      "Issue",
    );
...
export async function addComment(
  ...
  return transaction(pool, async (db) => {
    const issue = required(
      (
        await db.query(
          "UPDATE issues SET updated_at=clock_timestamp(),version=version+1 WHERE id=$1 RETURNING key",
          [issueId],
        )
      ).rows[0],
      "Issue",
    ...
    const id = randomUUID();
    await db.query(
      "INSERT INTO comments (id,issue_id,author_id,text) VALUES ($1,$2,$3,$4)",
      [id, issueId, actorId, text],
    );
    await logActivity(db, actorId, `Comment added on "${issue.key}"`);
    return (
      await db.query(
        `SELECT ${commentColumns} FROM comments c JOIN users u ON u.id=c.author_id WHERE c.id=$1`,
        [id],
      )
    ).rows[0];
  });
...
export async function deleteComment(
  ...
  return transaction(pool, async (db) => {
    required(
      (
        await db.query("SELECT id FROM issues WHERE id=$1 FOR UPDATE", [
          issueId,
        ])
      ).rows[0],
      "Issue",
