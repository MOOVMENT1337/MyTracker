...
async function freePort() {
  ...
  if (!address || typeof address === "string")
    throw new Error("No TCP port allocated");
...
export async function testDatabase() {
  ...
  if (!url) {
    ...
    const installed =
      ...
      existsSync("C:/Program Files/PostgreSQL/17/bin/initdb.exe")
        ? "C:/Program Files/PostgreSQL/17/bin"
        : "";
    const bin = configured || installed;
    const binary = (name: string) =>
      bin ? join(bin, name + suffix) : name + suffix;
    ...
    try {
      ...
      throw new Error(
        "Install PostgreSQL and set PG_BIN, or provide TEST_DATABASE_URL for a dedicated test database. No mock DB fallback.",
      );
    }
    ...
    stop = async () => {
      if (startAttempted) {
        try {
          ...
          /* It may have failed before creating postmaster.pid. */
        }
      }
      // Only this freshly created, exact temporary directory may be removed.
      const target = resolve(scratch);
      if (
        dirname(target) !== resolve(tempRoot) ||
        !basename(target).startsWith("tracker-pg-test-")
      )
  ...
  return {
    ...
    async close() {
      await pool.end();
      try {
        if (!/^tracker_test_[0-9a-f]{24}$/.test(schema))
