...
test("PATCH never applies create defaults to omitted fields", () => {
  assert.deepEqual(issuePatchSchema.parse({ summary: "New" }), {
    summary: "New",
  });
  assert.deepEqual(issuePatchSchema.parse({ assigneeId: null }), {
    assigneeId: null,
  });
...
test("query validation handles repeated filters and boolean strings", () => {
  ...
  assert.deepEqual(filterSchema.parse({ status: ["Open", "Done"] }).status, [
    "Open",
    "Done",
  ]);
...
test("queue keys and initials match the demo", () => {
  ...
  for (const key of ["A", "ABCDEFGHIJ"])
    assert.equal(queueSchema.parse({ name: "Demo", key }).key, key);
  for (const key of ["", "ABCDEFGHIJK", "DEV1", "DEV_QA", "DEV-QA", "ДЕВ"]) {
    assert.equal(
      queueSchema.safeParse({ name: "Demo", key }).success,
      false,
      key,
    );
  }
  assert.equal(initials("Alice Johnson"), "AJ");
  assert.equal(initials("Admin"), "AD");
});
test("local avatar palette follows the actual demo over two complete cycles", () => {
  const source = readFileSync(
    new URL("../../../html+css+js/scripts/data.js", import.meta.url),
    "utf8",
  );
