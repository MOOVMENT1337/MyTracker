...
Репозиторий подготовлен для воспроизводимого запуска frontend и API на NuxtCloud VPS через Docker Compose. Один образ собирает React-клиент и Express API, Caddy автоматически обслуживает HTTPS, а PostgreSQL размещается в Supabase. Защищённое подключение и миграции базы настраиваются автоматически при запуске API.
...
Потребуются домен, направленный на IP сервера, и URI **Session pooler** из Supabase на порту `5432`. Первый запуск создаёт файл конфигурации; после указания домена и строки подключения повторный запуск проверяет Supabase, собирает и запускает приложение. Полная инструкция, обновление и диагностика описаны в [руководстве по деплою](deploy/README.md).
...
Для размещения в интернете используйте готовую production-конфигурацию из [руководства по деплою](deploy/README.md). Она настраивает HTTPS, frontend и API на NuxtCloud, а PostgreSQL подключает из Supabase. Локальный `TrackerWebApp/Server/Dockerfile` собирает только API, а корневой production `Dockerfile` — API и frontend вместе.
