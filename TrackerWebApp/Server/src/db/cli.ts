...
try {
  switch (process.argv[2]) {
    case "migrate":
      ...
      break;
    case "seed":
      console.log(await seedAdmin(pool, config));
      break;
    case "seed-full-demo":
      console.log(await seedDemo(pool, config));
      break;
    case "reset":
      console.log(await resetWorkspace(pool, process.env.KEEP_ADMIN_ID));
      break;
    case "admin": {
      const data = z
        .object({
      ...
      await transaction(pool, (db) =>
        insertUser(db, {
          ...
          displayName: data.ADMIN_DISPLAY_NAME,
          passwordHash,
          isAdmin: true,
        }),
      );
    ...
    default:
      throw new Error(
        "Expected migrate, seed, seed-full-demo, reset, or admin",
      );
  }
} catch (error) {
