...
export async function insertUser(
  ...
  await db.query(
    `INSERT INTO users (id,email,display_name,password_hash,initials,avatar,provider,is_admin,role,avatar_color)
    ...
    [
      ...
      input.isAdmin || false,
      input.isAdmin ? "Administrator" : "Employee",
      userAvatarColor(count, input.provider),
    ],
  );
...
export async function issueSession(
  ...
  await db.query(
    ...
    [digest(accessToken), user.id, expiresAt],
  );
...
export async function login(
  ...
  let row = (
    await pool.query("SELECT id,password_hash FROM users WHERE email=$1", [
      normalized,
    ])
  ).rows[0];
  if (!row) {
    ...
    const candidates = (
      await pool.query("SELECT id,display_name FROM users")
    ).rows.filter((user) => user.display_name.toLowerCase() === normalized);
    if (candidates.length > 1) {
      throw new ApiError(
        400,
        "LOGIN_EMAIL_REQUIRED",
        "Multiple users have this display name; sign in with your email",
      );
    }
    if (candidates[0])
      row = (
  ...
  if (!(await verifyPassword(password, row?.password_hash ?? null)))
    throw new ApiError(
      401,
      "INVALID_CREDENTIALS",
...
export async function authenticate(pool: Database, authorization?: string) {
  ...
  if (!match)
    throw new ApiError(401, "UNAUTHORIZED", "Bearer authentication required");
...
export async function updateUser(
  ...
  actor: PublicUser,
  id: string,
  changes: { displayName?: string; role?: string },
) {
  requireAdmin(actor);
  if (changes.role && id === actor.id)
    throw new ApiError(403, "SELF_ROLE_CHANGE", "Cannot change your own role");
  return transaction(pool, async (db) => {
    ...
    if (changes.displayName) {
      ...
      await db.query(
        `UPDATE users SET display_name=$2, avatar=CASE WHEN avatar=initials THEN $3 ELSE avatar END, initials=$3 WHERE id=$1`,
        [id, changes.displayName, letters],
      );
    }
    if (changes.role)
      await db.query("UPDATE users SET role=$2 WHERE id=$1", [
        id,
        roleAliases[changes.role] || changes.role,
      ]);
