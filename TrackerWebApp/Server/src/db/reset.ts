...
export async function resetWorkspace(pool: Database, keepAdminId?: string) {
  return transaction(pool, async (db) => {
    await db.query(
      "LOCK TABLE users, queues, issues, comments, activity_log, sessions, oauth_identities, oauth_states IN ACCESS EXCLUSIVE MODE",
    );
    const admins = (
      await db.query(
        "SELECT id,email,password_hash FROM users WHERE is_admin=true ORDER BY created_at,id",
      )
    ).rows;
    const admin = keepAdminId
      ? admins.find((user) => user.id === keepAdminId)
      : admins.length === 1
        ? admins[0]
        : undefined;
    if (!admin)
      throw new Error(
        "Expected one administrator. If there are multiple admins, set KEEP_ADMIN_ID to the administrator to preserve. Nothing was deleted.",
      );
    if (!admin.password_hash)
      throw new Error(
        "The retained administrator must have a password login. Nothing was deleted.",
      );
    ...
    await db.query("UPDATE users SET theme='dark',language='ru' WHERE id=$1", [
      admin.id,
    ]);
