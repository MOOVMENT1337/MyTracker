...
const login = (identifier: string, password: string) =>
  request("/api/auth/login", { body: { identifier, password } });
const userCount = async () =>
  Number((await db.pool.query("SELECT count(*) FROM users")).rows[0].count);
...
test("display-name login is case-insensitive, supports Unicode and rejects wrong passwords", async () => {
  assert.equal(
    (await login("  ALICE JOHNSON  ", "pass123")).body.data.user.id,
    "u1",
  );
  ...
  assert.equal(
    (await login("ИВАН ПЕТРОВ", "secure-password")).body.data.user.id,
...
test("duplicate display names are allowed, but login must use a unique email", async () => {
  ...
  assert.equal(
    (await login("FIRST@EXAMPLE.COM", "first-password")).body.data.user.id,
    first.body.data.user.id,
  );
  assert.equal(
    (await login("second@example.com", "second-password")).body.data.user.id,
    second.body.data.user.id,
  );
  assert.equal(
    (await register("Different Name", " FIRST@EXAMPLE.COM ")).status,
    409,
  );
  assert.equal(
    (await login("first@example.com", "second-password")).status,
    401,
  );
});
test("email takes precedence over another account display name", async () => {
  ...
  assert.equal(
    (await login("owner@example.com", "secure-password")).body.data.user.id,
    owner.body.data.user.id,
  );
  assert.equal(
    (await login("owner@example.com", "namesake-password")).status,
    401,
  );
});
test("renamed users login with their new names, including the demo administrator", async () => {
  const renamed = await request("/api/users/admin1", {
    method: "PATCH",
    token: adminToken,
  ...
  try {
    assert.equal(
      (await login("Renamed Admin", "admin123")).body.data.user.id,
      "admin1",
    );
    assert.equal((await login("Admin", "admin123")).status, 401);
    assert.equal(
      (await login("admin@tracker.com", "admin123")).body.data.user.id,
      "admin1",
    );
  } finally {
    assert.equal(
...
test("registrations cycle through the demo palette and failed registration does not consume a color", async () => {
  ...
  for (let index = 0; index < 9; index++) {
    const created = await register(
      `Palette ${index}`,
      `palette-${index}@example.com`,
    );
    assert.equal(created.status, 201);
    assert.equal(
      created.body.data.user.avatarColor,
      colors[(start + index) % colors.length],
    );
    assert.equal(
      (await register("Duplicate", `palette-${index}@example.com`)).status,
      409,
    );
  }
});
test("concurrent registrations allocate consecutive palette positions and email remains unique", async () => {
  ...
  const results = await Promise.all(
    Array.from({ length: 8 }, (_, index) =>
      register(`Concurrent ${index}`, `parallel-${index}@example.com`),
    ),
  );
  assert.ok(results.every((result) => result.status === 201));
  assert.deepEqual(
    results.map((result) => result.body.data.user.avatarColor).sort(),
    Array.from(
      { length: 8 },
      (_, index) => colors[(start + index) % colors.length],
    ).sort(),
  );
  const duplicate = await Promise.all([
    register("One", "concurrent-duplicate@example.com"),
    register("Two", "CONCURRENT-DUPLICATE@EXAMPLE.COM"),
  ]);
...
test("single-letter and ten-letter queue keys work through API, numbering and lookup", async () => {
  for (const key of ["a", "abcdefghij"]) {
    const queue = await request("/api/queues", {
      token: adminToken,
      body: { name: "Demo queue", key },
    });
    assert.equal(queue.status, 201, JSON.stringify(queue.body));
    const issue = await request("/api/issues", {
      token: adminToken,
      body: { queueId: queue.body.data.id, summary: "Demo issue" },
    });
    assert.equal(issue.status, 201);
    assert.equal(issue.body.data.key, `${key.toUpperCase()}-1`);
    assert.equal(
      (await request(`/api/issues/by-key/${key}-1`, { token: adminToken })).body
        .data.id,
      issue.body.data.id,
    );
    assert.equal(
      (await request(`/api/queues/by-key/${key}`, { token: adminToken })).body
        .data.id,
      queue.body.data.id,
    );
    assert.equal(
      (
        await request("/api/queues", {
          token: adminToken,
          body: { name: "Duplicate", key: key.toUpperCase() },
        })
      ).status,
      409,
    );
