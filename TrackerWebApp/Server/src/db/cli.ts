...
try {
  switch (process.argv[2]) {
    ...
    case 'seed': console.log(await seedDemo(pool, config)); break;
    case 'admin': {
      const data = z.object({
      ...
      await insertUser(pool, { email: data.ADMIN_EMAIL, displayName: data.ADMIN_DISPLAY_NAME,
        passwordHash: await hashPassword(data.ADMIN_PASSWORD), isAdmin: true });
      console.log('Administrator created. Existing users were not modified.');
      break;
    }
    default: throw new Error('Expected migrate, seed, or admin');
  }
} catch (error) {
