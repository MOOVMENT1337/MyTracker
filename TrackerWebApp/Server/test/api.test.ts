...
test("migrations and seed are repeatable and production seed is blocked", async () => {
  ...
  await assert.rejects(
    () => seedDemo(db.pool, { ...config, NODE_ENV: "production" }),
    /non-production/,
  );
  assert.equal(
    Number(
...
test("all 6 demo users, 3 queues, 12 issues and 9 comments are preserved", async () => {
  ...
  assert.equal(
    (await request("/api/queues", { token: aliceToken })).body.data.length,
    3,
  );
  ...
  assert.ok(
    ...
    ).rows[0].password_hash.startsWith("scrypt$"),
  );
});
test("protected routes reject missing, fake and expired sessions", async () => {
  ...
  assert.equal(
    (await request("/api/auth/me", { token: accessToken })).status,
    401,
  );
});
test("admin creates users without a session; public registration and privilege injection are rejected", async () => {
  ...
  assert.equal(
    (await request("/api/auth/register", { method: "POST", body })).status,
    403,
  );
  assert.equal(
    (await request("/api/users", { method: "POST", body })).status,
    401,
  );
  assert.equal(
    (await request("/api/users", { method: "POST", token: aliceToken, body }))
      .status,
    403,
  );
  ...
  assert.equal(
    (await request("/api/users", { method: "POST", token: adminToken, body }))
      .status,
    409,
  );
  assert.equal(
  ...
  assert.equal(
    (
      await request("/api/auth/login", {
        method: "POST",
        body: { identifier: body.email, password: "wrong" },
      })
    ).status,
    401,
  );
...
test("server enforces admin edits, custom role aliases, and no role-based escalation", async () => {
  ...
  const result = await request("/api/users/u2", {
    method: "PATCH",
    token: adminToken,
    body: { displayName: "Robert Smith", role: "Тестировщик" },
  });
  assert.equal(result.body.data.displayName, "Robert Smith");
  assert.equal(result.body.data.avatar, "RS");
  assert.equal(result.body.data.role, "Tester");
  const titleOnly = await request("/api/users/u2", {
    method: "PATCH",
    token: adminToken,
    body: { role: "Administrator" },
  });
  ...
  assert.equal(
    (
      await request("/api/users/admin1", {
        method: "PATCH",
        token: adminToken,
        body: { role: "Employee" },
      })
...
test("settings are isolated per user and survive new API instances", async () => {
  ...
  assert.deepEqual(
    (await request("/api/settings", { token: bobToken })).body.data,
    { theme: "dark", language: "ru" },
  );
  await stopServer();
  await listen();
  assert.deepEqual(
    (await request("/api/settings", { token: aliceToken })).body.data,
    { theme: "light", language: "en" },
  );
  assert.equal(
    (await request("/api/auth/me", { token: aliceToken })).body.data.id,
    "u1",
  );
});
test("queue creation, normalized keys, lookup, duplicates and validation", async () => {
  const result = await request("/api/queues", {
    method: "POST",
    token: aliceToken,
    body: { name: "API Tests", key: "test" },
  });
  assert.equal(result.status, 201);
  queueId = result.body.data.id;
  assert.equal(result.body.data.key, "TEST");
  assert.equal(
    (await request("/api/queues/by-key/test", { token: aliceToken })).body.data
      .id,
    queueId,
  );
  assert.equal(
...
test("issue creation generates key, reporter and timestamps on server", async () => {
  const result = await request("/api/issues", {
    ...
    body: {
      ...
      assigneeId: "u1",
      description: "Literal 100%_text",
      priority: "High",
    },
  });
  assert.equal(result.status, 201, JSON.stringify(result.body));
  issueId = result.body.data.id;
  assert.equal(result.body.data.key, "TEST-1");
  assert.equal(result.body.data.reporterId, "u1");
  assert.equal(result.body.data.version, 1);
  assert.ok(result.body.data.statusChangedAt);
  assert.equal(
    (await request("/api/issues/by-key/test-1", { token: aliceToken })).body
      .data.id,
    issueId,
  );
  assert.equal(
    (
      await request("/api/issues", {
        method: "POST",
        token: aliceToken,
        body: { queueId, summary: "forged", reporterId: "admin1" },
      })
    ).status,
    400,
  );
...
test("invalid references roll back issue and counter together", async () => {
  const result = await request("/api/issues", {
    method: "POST",
    token: aliceToken,
    body: { queueId, summary: "Bad reference", assigneeId: "absent-user" },
  });
  assert.equal(result.status, 400);
  assert.equal(
    (await request(`/api/queues/${queueId}`, { token: aliceToken })).body.data
      .counter,
    1,
  );
  assert.equal(
    (
      await request("/api/issues", {
        method: "POST",
        token: aliceToken,
        body: { queueId: "absent-queue", summary: "Bad queue" },
      })
    ).status,
    404,
  );
...
test("concurrent issue creation allocates 16 distinct sequential keys", async () => {
  const results = await Promise.all(
    Array.from({ length: 16 }, (_, index) =>
      request("/api/issues", {
        method: "POST",
        token: bobToken,
        body: { queueId, summary: `Concurrent ${index}` },
      }),
    ),
  );
  assert.ok(
  ...
  const numbers = results
    .map((r) => Number(r.body.data.key.split("-")[1]))
    .sort((a, b) => a - b);
  assert.deepEqual(
    numbers,
...
test("partial issue edits preserve omitted fields and reset timer only on status change", async () => {
  const before = (
    await request(`/api/issues/${issueId}`, { token: aliceToken })
  ).body.data;
  const edit = await request(`/api/issues/${issueId}`, {
    method: "PATCH",
    token: aliceToken,
    body: { summary: "Renamed API issue", version: before.version },
  });
  assert.equal(edit.status, 200);
  assert.equal(edit.body.data.priority, "High");
  assert.equal(edit.body.data.description, before.description);
  assert.equal(edit.body.data.statusChangedAt, before.statusChangedAt);
  const status = await request(`/api/issues/${issueId}`, {
    method: "PATCH",
    token: aliceToken,
    body: { status: "Done" },
  });
  assert.equal(status.body.data.status, "Done");
  assert.notEqual(status.body.data.statusChangedAt, before.statusChangedAt);
  assert.equal(
    (
      await request(`/api/issues/${issueId}`, {
        method: "PATCH",
        token: aliceToken,
        body: { summary: "stale", version: 1 },
      })
    ).status,
    409,
  );
...
test("two writes with same version cannot silently overwrite each other", async () => {
  const issue = (await request(`/api/issues/${issueId}`, { token: aliceToken }))
    .body.data;
  const results = await Promise.all(
    ["A", "B"].map((summary) =>
      request(`/api/issues/${issueId}`, {
        method: "PATCH",
        token: aliceToken,
        body: { summary, version: issue.version },
      }),
    ),
  );
...
test("comments use authenticated author and enforce ownership even for admin", async () => {
  const result = await request(`/api/issues/${issueId}/comments`, {
    method: "POST",
    token: aliceToken,
    body: { text: "API comment" },
  });
  assert.equal(result.status, 201);
  assert.equal(result.body.data.authorId, "u1");
  const path = `/api/issues/${issueId}/comments/${result.body.data.id}`;
  assert.equal(
    (await request(path, { method: "DELETE", token: bobToken })).status,
    403,
  );
  assert.equal(
    (await request(path, { method: "DELETE", token: adminToken })).status,
    403,
  );
  assert.equal(
    (await request(path, { method: "DELETE", token: aliceToken })).status,
    204,
  );
  assert.equal(
    (await request(path, { method: "DELETE", token: aliceToken })).status,
...
test("filters, literal search, multi-select, myTasks and full-set statistics", async () => {
  const mine = await request(
    "/api/issues?myTasks=true&status=Open&status=Done&priority=High&limit=1",
    { token: aliceToken },
  );
  assert.equal(mine.status, 200);
  assert.equal(mine.body.data.length, 1);
  assert.ok(mine.body.pagination.total >= 2);
  const literal = await request("/api/issues?search=100%25_text", {
    token: aliceToken,
  });
  assert.equal(literal.body.pagination.total, 1);
  assert.equal(
    (
      await request("/api/issues?search=%27%20OR%201%3D1--", {
        token: aliceToken,
      })
    ).body.pagination.total,
    0,
  );
  const stats = await request(`/api/issues/stats?queueId=${queueId}&limit=1`, {
    token: aliceToken,
  });
  assert.equal(stats.body.data.total, 17);
  assert.equal(stats.body.data.byStatus.Done, 1);
  assert.equal(
    (await request("/api/issues?limit=101", { token: aliceToken })).status,
    400,
  );
  assert.equal(
    (await request("/api/issues?offset=999999", { token: aliceToken })).body
      .data.length,
    0,
  );
});
test("shared activity includes creates, comments and status history", async () => {
  ...
  assert.ok(
    result.body.data.some((entry: any) => entry.msg.includes("Open → Done")),
  );
  assert.ok(
    result.body.data.some((entry: any) => entry.msg.includes("Comment added")),
  );
  const metadata = (await request("/api/metadata", { token: aliceToken })).body
    .data;
...
test("deleting issues cascades comments and does not reuse issue keys", async () => {
  ...
  assert.equal(
    Number(
      (
        await db.pool.query("SELECT count(*) FROM comments WHERE issue_id=$1", [
          issueId,
        ])
      ).rows[0].count,
    ),
  ...
  const newIssue = await request("/api/issues", {
    method: "POST",
    token: aliceToken,
    body: { queueId, summary: "After deletion" },
  });
...
test("only admin can delete a queue; all children are removed atomically", async () => {
  ...
  const result = await request(`/api/queues/${queueId}`, {
    method: "DELETE",
    token: adminToken,
  });
  assert.equal(result.status, 200);
  assert.equal(result.body.data.issueCount, 17);
  assert.equal(
    Number(
      (
        await db.pool.query("SELECT count(*) FROM issues WHERE queue_id=$1", [
          queueId,
        ])
      ).rows[0].count,
    ),
  ...
  assert.equal(
    (await request(`/api/queues/${queueId}`, { token: aliceToken })).status,
    404,
  );
});
test("CORS, malformed JSON, mass assignment, media type and body limits are enforced", async () => {
  ...
  assert.ok(!JSON.stringify(missing.body).includes("stack"));
});
...
test("browser sessions survive refresh, omit bearer tokens, enforce CSRF and revoke on logout", async () => {
  ...
  const session = await request("/api/auth/login", {
    method: "POST",
    headers: { "X-Tracker-Browser": "1", Origin: "http://localhost:5173" },
    body: { identifier: "admin@tracker.com", password: "admin123" },
  });
  ...
  assert.equal(
    (await request("/api/auth/me", { headers: { Cookie } })).body.data.id,
    "admin1",
  );
  assert.equal(
    (
  ...
  assert.equal(
    (
      await request("/api/settings", {
        method: "PATCH",
        headers: { Cookie, "X-Tracker-Browser": "1" },
        body: { theme: "dark" },
      })
    ).status,
    200,
  ...
  assert.equal(
    (await request("/api/auth/me", { headers: { Cookie } })).status,
    401,
  );
});
...
test("external sign-in stays disabled even with provider credentials configured", async () => {
  ...
  const before = (
    await db.pool.query("SELECT count(*)::int AS count FROM sessions")
  ).rows[0].count;
  for (const provider of ["google", "yandex", "mail"]) {
    for (const path of [
      `/api/auth/oauth/${provider}`,
      `/api/auth/oauth/${provider}/callback?code=fake&state=${"a".repeat(43)}`,
    ]) {
  ...
  assert.equal(
    (await db.pool.query("SELECT count(*)::int AS count FROM sessions")).rows[0]
      .count,
    before,
  );
});
