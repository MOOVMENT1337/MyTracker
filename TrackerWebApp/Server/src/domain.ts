...
export const socialAvatarColors = {
  google: "#DB4437",
  yandex: "#FC3F1D",
  mail: "#168DE2",
} as const;
...
export function userAvatarColor(
  ...
  return provider === "local"
    ? userColorPalette[userCount % userColorPalette.length]!
    : socialAvatarColors[provider];
}
...
export const issuePatchSchema = z
  ...
  .refine(
    ...
    "At least one changed field required",
  );
export const userPatchSchema = z
  .strictObject({ displayName: name.optional(), role: name.optional() })
  .refine(
...
const multi = <T extends readonly [string, ...string[]]>(values: T) =>
  z
    .union([z.enum(values), z.array(z.enum(values)).min(1).max(values.length)])
    .optional();
export const filterSchema = paginationSchema
  .extend({
    queueId: idSchema.optional(),
    status: multi(statuses),
    priority: multi(priorities),
    type: multi(issueTypes),
...
// Public domain types and errors
